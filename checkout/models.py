...
class Order(models.Model):
    ...
    def _generate_order_number(self):
        """
        Generate a random but unique order number using UUID
        """
...
class OrderPerItem(models.Model):
    order = models.ForeignKey(
        Order, null=False, blank=False,
        on_delete=models.CASCADE, related_name='peritems')
    product = models.ForeignKey(
        Product, null=False, blank=False, on_delete=models.CASCADE)
    nicotine_level = models.CharField(
        max_length=2, null=True, blank=True)  # 3mg, 6mg, 10mg, 18mg
    quantity = models.IntegerField(
        null=False, blank=False, default=0)
    peritem_total = models.DecimalField(
